...
if [ -e final_project_src.zip ]; then
  echo "final_project_src.zip.  Good."
  if zipinfo -1 final_project_src.zip | grep -q "^state.ml$"; then
  ...
  echo "final_project_src.zip does NOT exist.  This is bad.  Have you run \"make zip\"?"
  zip=bad
fi
...
EOF
  zipinfo -1 final_project_src.zip
else
  cat <<EOF
===========================================================
WARNING
